...
const { modlogc, criminalsc, jailc, jailr, friendr, errorc } = require('../../../config.json');
...
module.exports = {
	...
	async execute(interaction) {
		try {
			...
			if (!friendr) await error.send(`Main role not found. Please do the setup`);
			if (!modlog || !criminals || !jail) await error.send(`modlog, criminals or jail not found, please do the setup`);
			...
			if (member.roles.cache.has(friendr)) { await criminals.send({ embeds: [crimembed ] }).catch(O_o => error.send(`${O_o}`)); }
			...
			await jail.send({ content: `${target} **Verify your age!**`, embeds: [ageembed] }).catch(O_o => error.send(`${O_o}`));
			...
			await modlog.send({ embeds: [modembed] }).catch(O_o => error.send(`${O_o}`));
