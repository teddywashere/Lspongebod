...
const { modlogc, criminalsc, muter, errorc, clientId } = require('../../../config.json');
...
module.exports = {
	...
	async execute(interaction) {
		try {
			...
			if (!modlog || !criminals) await error.send(`Either modlog or criminals not found. Please do the setup`);
			...
			await modlog.send({ embeds: [muteembed] }).catch(O_o => error.send(`${O_o}`));
			await criminals.send({ embeds: [crimeembed] }).catch(O_o => error.send(`${O_o}`));
