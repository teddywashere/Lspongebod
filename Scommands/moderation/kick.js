...
module.exports = {
	...
	async execute(interaction) {
		try {
			interaction.reply({ content: `Kicking...`, ephemeral: true });
			...
			if (!modlog || !criminals) await me.send(`${interaction.user} tried to use kick command in ${interaction.guild}\n**ERROR:**\nEither modlog or criminals not found`).catch(O_o => console.log(O_o));
			...
			await modlog.send({ embeds: [kickembed] }).catch(O_o => me.send(`${interaction.user} tried to use kick command in ${interaction.guild}\n**ERROR:**\n${O_o}`));
			...
			await criminals.send({ embeds: [ripembed] }).catch(O_o => me.send(`${interaction.user} tried to use kick command in ${interaction.guild}\n**ERROR:**\n${O_o}`));
